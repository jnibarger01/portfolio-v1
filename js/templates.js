...
import { routePath } from './platform.js';
...
<p class="muted">Portal ${index + 1} of ${total} · <a href="${esc(routePath(`project/${p.slug}`))}">/project/${esc(p.slug)}</a></p>`;
}
...
<div><h4><a href="${esc(routePath(`project/${p.slug}`))}" data-travel="${esc(p.slug)}">${esc(p.name)}</a>${discovered.has(p.slug) ? '<span class="seen-tag">VISITED</span>' : ''}</h4><p>${esc(p.tagline)}</p></div>
...
<p>${esc(c.role)}</p>
<p>${esc(c.story || '')}</p>
<div class="actions"><a class="btn is-primary" href="${esc(routePath('about'))}" data-open-tab="home">More about Jace</a><button class="btn" type="button" data-close>Keep driving</button></div>`;
}
